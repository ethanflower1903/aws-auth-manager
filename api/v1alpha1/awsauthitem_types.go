...
type AWSAuthItemSpec struct {
	// DryRun tells the controller to render the AWSAuthItem
	// into the dry-run configmap. Default to false.
...
// AWSAuthItemReady registers a successful reconciliation of the given AWSAuthItem.
func AWSAuthItemDryRunReady(item AWSAuthItem) AWSAuthItem {
	meta.SetResourceCondition(&item, meta.ReadyCondition, metav1.ConditionTrue, DryRunSuccededReason,
		"DryRun reconciliation succeeded")
	return item
}
